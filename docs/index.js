...
const cardBackImgPath = './src/card-AceSpades.png'

const cardContainerElem = document.querySelector('.card-container')

let cards = []

//this variable will be used as a reference to the play game button
const playGameButtonElem = document.getElementById('playGame')

//change the grid to one element instead of four
const collapsedGridAreaTemplate = '"a a" "a a"'

//reprstenting the grid cell after collapsing the grid
const cardCollectionCellClass = ".card-pos-a"

const numCards = cardObjectDefinitions.length

//variable to store an array of cards positions
let cardPositions = [ ]
...
const scoreContainerElem = document.querySelector('.header-score-container') //stores the container element
const scoreElem = document.querySelector('.score') //stores the score element 
...
// this function check if the card selected is the Ace of spades
...
{
    //card can be clicked when not in shuffle and the card is not turned
    return gameInProgress == true && !shufflingInProgress && !cardsRevealed
}

function loadGame() {
    createCards()
    //assign card elements to the array
    cards = document.querySelectorAll('.card')

    cardFlyInEffect()

    //event listener for play game button to call startGame function
...
function initializeNewGame() {
    ...
    //make the round and score visible after the game started
    updateStatusElement(scoreContainerElem, "flex")
    updateStatusElement(roundContainerElem, "flex")

    //and change the score and round
    updateStatusElement(scoreElem,"block", primaryColor, `Score <span class='badge'>${score}</span>`)
...
function transformGridArea(lemon)
{
    cardContainerElem.style.gridTemplateAreas = lemon
...
function randomisedCardPositions() {
    const random1 = Math.floor(Math.random() * numCards) + 1
    const random2 = Math.floor(Math.random() * numCards) + 1
...
function createCard(cardItem) {
    ...
    //create a front and back image element for a card
    ...
    //add back image element as chld element to back card elemenet
    addChildElement(cardBackElem, cardBackImg)
    
    //add front image element as chld element to front card elemenet
    addChildElement(cardFrontElem, cardFrontImg)

    //add front card element as child element to innser card element 
    addChildElement(cardInnerElem, cardFrontElem)
    
    //add back card element as child element to innser card element 
    ...
    initialiseCardPostions(cardElem)

    attachClickEventHandler(cardElem)
    
}

function attachClickEventHandler(card) {
    card.addEventListener('click', () => chooseCard(card))
}

//method that store the initial positions of the card in the grid the gameplay area
function initialiseCardPostions(card) {
    cardPositions.push(card.id)
}

// Reusable function to creating an element 
function createElement(elemType) {
    return document.createElement(elemType)
}
//Reusable method that add a class to HTML element
function addClassToElement(elem, className) {
     elem.classList.add(className)
}
//Reusable method that create a unique ID to the card
